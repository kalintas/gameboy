...
impl GoToLinePopup {
    ...
    pub fn render(&mut self, ui: &imgui::Ui, scroll_to_line: impl FnOnce(f32) -> u32, line_to_scroll: impl FnOnce(u32) -> f32) {
        
        if ui.is_window_focused() && ui.is_key_down(imgui::Key::LeftCtrl) && ui.is_key_down(imgui::Key::G) {
        ...
        if let Some(line) = ui.modal_popup_config(self.name)
            .movable(false)
            .resizable(false)
            .build(|| {
